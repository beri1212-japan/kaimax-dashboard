...
**未確認のリスク: MOTA の加盟店規約で自動収集が許容されているか。**
規約違反ならアカウント停止 = システム停止。契約書の確認と、公式データ連携APIの有無の問い合わせが必要。
...
1. ワーカーの `.gitignore` 作成（`.env.local` の漏洩防止）
2. **MOTA の加盟店規約を確認**し、自動収集の可否と公式データ連携APIの有無を問い合わせる ← ご本人のみ
